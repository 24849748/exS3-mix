...
encoder_handle_t encoder_create(pcnt_unit_t uint, int PIN_A, int PIN_B){
    ...
    /* 开启pcnt计数、过滤，记录最后一次读数 */
    // encoder_start(ecd);
    // encoder_set_glitch_fliter(ecd, 1);
    // ecd->last_count = encoder_get_counter_value(ecd);
...
int encoder_get_counter_value(encoder_handle_t ecd){
    ...
    if(_ecd->last_count < read_count){
        _ecd->diff = 1;
    }else if(_ecd->last_count > read_count){
        _ecd->diff = -1;
    }else if(_ecd->last_count == read_count){
        _ecd->diff = 0;
    }
    
    _ecd->last_count = read_count;
    ESP_LOGI(TAG, "diff: %d, count: %d",_ecd->diff, read_count);
...
//  * @return int 
//  */
// int encoder_get_diff(encoder_handle_t ecd){
//     encoder_t *_ecd = (encoder_t *)ecd;
//     int16_t read_count = 0;
//     read_count = encoder_get_counter_value(ecd);
//     if(read_count - last_count < 0 && read_count - last_count <= -3){
//         _ecd->diff = -1;
//     }else if(read_count - last_count > 0 && read_count - last_count >3){
//         _ecd->diff = 1;
//     }
// }
